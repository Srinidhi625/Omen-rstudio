...
#
# Description: This R script trains a model using two separate datasets: one for
# training and one for testing. It preprocesses the audio data by extracting MFCCs,
# trains a Convolutional Neural Network (CNN) using the 'torch' package, and
# saves the trained model. This script has NO PYTHON DEPENDENCY.
#
...
# requires a one-time installation of its core libraries.
#
...
#
# This section assumes the dataset has already been extracted.
...
#
# This section runs on the combined dataset.
# You can now change these values, and the model will adapt automatically.
#
...
Net <- nn_module(
  ...
  initialize = function(n_mfcc_val, max_pad_len_val) {
    ...
    # Each max_pool2d(2) halves the dimensions. There are 3 pooling layers.
    final_height <- floor(n_mfcc_val / 8)
    final_width <- floor(max_pad_len_val / 8)
    flattened_size <- 128 * final_height * final_width
    
    self$fc1 <- nn_linear(in_features = flattened_size, out_features = 128)
...
# Instantiate the model, passing the feature parameters to the constructor
model <- Net(n_mfcc_val = N_MFCC, max_pad_len_val = MAX_PAD_LEN)
